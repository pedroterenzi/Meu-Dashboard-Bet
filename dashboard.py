...
st.markdown("""
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    /* Grid do Calendário */
    .calendar-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 15px; margin-top: 20px; }
    ...
    .day-stks { font-size: 0.75rem; opacity: 0.8; }
    ...
    .perf-value { flex: 1; text-align: center; font-weight: 800; font-size: 1.1rem; }
    .val-green { color: #10b981; }
    .val-red { color: #ef4444; }
    .perf-roi { flex: 1.5; padding: 0 20px; }
    .roi-bar-bg { background: #334155; border-radius: 10px; height: 8px; width: 100%; overflow: hidden; position: relative; }
    .roi-bar-fill { height: 100%; border-radius: 10px; }

    /* Métricas Topo */
    .metric-card { padding: 20px; border-radius: 20px; text-align: center; color: white; font-weight: 800; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3); margin-bottom: 20px; }
    </style>
...
# --- 2. BARRA LATERAL ---
st.sidebar.header("🕹️ Painel")
arquivo_path = st.sidebar.text_input("Caminho do CSV", r'C:\Users\pedro-santos\Downloads\AccountStatement_ (1).csv')
...
try:
    ...
    if len(data_sel) == 2:
        ...
        # Extrair ID das referências se houver
        def extract_id(row):
        ...
        for dia in dias_mes:
            if dia == 0: html_cal += '<div class="empty-card"></div>'
                ...
                if valor is None: content = f'<div class="day-number">{dia}</div><div class="day-off">OFF</div>'
                    ...
                    content = f'<div class="day-number">{dia}</div><div class="day-value">+R$ {valor:,.2f}</div><div class="day-stks">+{(valor/stake_padrao):.2f} STK</div>'
                elif valor < -0.05:
                    card_class += " red-card"
                    content = f'<div class="day-number">{dia}</div><div class="day-value">-R$ {abs(valor):,.2f}</div><div class="day-stks">-{(abs(valor)/stake_padrao):.2f} STK</div>'
                else: content = f'<div class="day-number">{dia}</div><div class="day-value">R$ 0.00</div>'
        ...
        def extrair_est(txt):
            txt = str(txt).split('Ref:')[0] # Remove o ID do final
            if '/' in txt: return txt.split('/')[-1].strip()
            return "Match Odds / Outros"
        ...
        for est, row in resumo.iterrows():
            ...
            <div class="perf-card">
                <div class="perf-info">
                    ...
                    <div class="perf-meta">{int(row['Entradas'])} Entradas • {row['Lucro']/stake_padrao:,.2f} STKS</div>
                </div>
                ...
                <div class="perf-roi">
                    <div style="display:flex; justify-content:space-between; font-size:0.75rem; margin-bottom:4px; color:#94a3b8">
                        <span>ROI</span><span>{row['ROI']:.2f}%</span>
                    </div>
                    <div class="roi-bar-bg">
                        <div class="roi-bar-fill" style="width: {w_roi}%; background: {c_roi};"></div>
                    </div>
                </div>
            </div>"""
        html_perf += '</div>'
